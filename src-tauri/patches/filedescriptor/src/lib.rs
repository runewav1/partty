...
pub trait FromRawFileDescriptor {
    /// Create an instance from the underlying platform file descriptor type.
    ///
    /// # Safety
    ///
    /// The `fd` must be a valid, owned handle for the resulting type. Taking
    /// ownership of a descriptor that is still referenced elsewhere, or of one
    /// that was never valid, may lead to double-close or use-after-close.
...
pub trait FromRawSocketDescriptor {
    /// Create an instance from the underlying platform socket descriptor type.
    ///
    /// # Safety
    ///
    /// The `fd` must be a valid, owned socket descriptor for the resulting
    /// type. Taking ownership of a socket that is still referenced elsewhere,
    /// or of one that was never valid, may lead to double-close or
    /// use-after-close.
