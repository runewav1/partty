...
export function createDevMetricsOverlay(
  ...
  `;
	...
	function updateGlobal(container: HTMLElement): void {
		...
		if (fps !== undefined) {
			const cls =
				fps < 30 ? "ov-metric--warn" : fps < 55 ? "ov-metric--mid" : "";
		...
		if (lastShift !== undefined)
			shiftLabel += ` (last ${lastShift.toFixed(3)})`;
		...
		if (taskTiming)
			taskLabel += ` (avg ${(taskTiming.totalMs / taskTiming.count).toFixed(0)}ms)`;
	...
	function tick(): void {
		...
		const globalContainer = el.querySelector<HTMLElement>(
			'[data-metrics="global"]',
		);
		const paneContainer = el.querySelector<HTMLElement>(
			'[data-metrics="panes"]',
		);
