...
import { expandRelativePath, quotePath, translatePasteText, translatePathFromSource } from "../util/paths.ts";
...
test("relative traversal preserves filesystem roots", () => {
	...
	assert.equal(expandRelativePath("./a\\b.txt", "/home/me", "posix"), "/home/me/a\\b.txt");
});
...
test("paste and editor translation use the source terminal's path style", () => {
	for (const [raw, source, target, expected] of [
		...
		assert.equal(translatePasteText(raw, target, { style: source, cwd }), quotePath(expected, target));
	}
	assert.equal(translatePasteText("/home/me/a", "windows", {
		style: "wsl", cwd: "\\\\wsl$\\Ubuntu",
	}), "\\\\wsl$\\Ubuntu\\home\\me\\a");
});

test("POSIX quoting protects shell syntax and preserves home expansion", () => {
	for (const path of ["/tmp/a;b", "/tmp/a&b", "/tmp/a(b)", "/tmp/[ab]*", "/tmp/a#b"]) {
		assert.equal(quotePath(path, "posix"), `'${path}'`);
	}
	assert.equal(quotePath("~/my files/a'b", "wsl"), "~/'my files/a'\\''b'");
	assert.equal(translatePasteText("ordinary text\n/another line", "wsl"), "ordinary text\n/another line");
});
